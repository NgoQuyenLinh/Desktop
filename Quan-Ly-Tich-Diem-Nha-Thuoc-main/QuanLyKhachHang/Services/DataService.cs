...
namespace QuanLyKhachHang.Services
{
    ...
    public class DataService
    {
        ...
        public void LuuThuoc() => GhiFile(_thuocFile, DanhSachThuoc);
        ...
        {
            int soThuTu = DanhSachThuoc.Count == 0
                ? 1
                : DanhSachThuoc
                    .Select(t => int.TryParse(t.MaThuoc.Replace("T", ""), out int n) ? n : 0)
                    .DefaultIfEmpty(0)
                    .Max() + 1;
            return $"T{soThuTu:D3}";
        }
        ...
        {
            var t = DanhSachThuoc.FirstOrDefault(x => x.MaThuoc == thuocMoi.MaThuoc);
            if (t == null) return false;

            t.TenThuoc = thuocMoi.TenThuoc;
            t.DonGia = thuocMoi.DonGia;
            t.ConHang = thuocMoi.ConHang;
            LuuThuoc();
            return true;
        }
        ...
        {
            var t = DanhSachThuoc.FirstOrDefault(x => x.MaThuoc == maThuoc);
            if (t == null) return false;

            DanhSachThuoc.Remove(t);
            LuuThuoc();
            return true;
        }

        /// <summary>Tìm kiếm tức thời theo tên / mã thuốc, không phân biệt hoa thường, chỉ cần chứa từ khoá.</summary>
        public List<Thuoc> TimKiemThuoc(string? tuKhoa = null)
        {
            var ds = DanhSachThuoc.AsEnumerable();

            if (!string.IsNullOrWhiteSpace(tuKhoa))
            {
                var tk = tuKhoa.Trim().ToLower();
                ds = ds.Where(t => t.TenThuoc.ToLower().Contains(tk) || t.MaThuoc.ToLower().Contains(tk));
            }

            return ds.OrderBy(t => t.TenThuoc).ToList();
        }

        /// <summary>
        /// Danh sách thuốc ĐỦ ĐIỀU KIỆN để chọn khi tạo đơn hàng: chỉ những thuốc
        /// đang ở trạng thái "Còn hàng" (Thuoc.ConHang == true) mới hiện ra ở đây.
        /// </summary>
        public List<Thuoc> ThuocConHang() =>
            DanhSachThuoc.Where(t => t.ConHang).OrderBy(t => t.TenThuoc).ToList();
        ...
        /// <summary>
        /// Tạo 1 đơn hàng mới cho khách hàng từ danh sách các thuốc đã chọn (1 đơn hàng
        /// có thể chứa nhiều loại thuốc, mỗi loại 1 dòng ChiTietDonHang), tự động:
        ///  - Tính Tổng tiền đơn hàng = tổng Thành tiền của các thuốc trong danh sách.
        ///  - Kiểm tra không cho dùng quá số điểm đang có.
        ///  - Trừ điểm đã dùng, cộng điểm mới (Điểm cộng = Tổng tiền / 1000, làm tròn xuống).
        ///  - Cập nhật lại điểm tích luỹ của khách hàng và ghi file.
        /// </summary>
        public (bool ThanhCong, string ThongBao, DonHang? Don) TaoDonHang(string maKH, List<ChiTietDonHang> danhSachThuoc, int diemSuDung, QuaTang? quaTang = null)
        {
            var kh = DanhSachKhachHang.FirstOrDefault(x => x.MaKH == maKH);
            if (kh == null)
            ...
            if (danhSachThuoc == null || danhSachThuoc.Count == 0)
                return (false, "Đơn hàng phải có ít nhất 1 loại thuốc.", null);

            if (danhSachThuoc.Any(t => t.SoLuong <= 0))
                return (false, "Số lượng thuốc không hợp lệ.", null);

            decimal soTien = danhSachThuoc.Sum(t => t.ThanhTien);
        ...
        public int TongSoDonHang() => DanhSachDonHang.Count;

        /// <summary>Tổng doanh thu từ thuốc = tổng Thành tiền của toàn bộ đơn hàng (mọi đơn hàng đều là bán thuốc).</summary>
        public decimal TongDoanhThuTuThuoc() => DanhSachDonHang.Sum(d => d.ThanhTien);

        /// <summary>Tổng số thuốc đã bán = tổng số lượng của tất cả các dòng thuốc trong tất cả đơn hàng.</summary>
        public int TongSoThuocDaBan() => DanhSachDonHang.Sum(d => d.DanhSachThuoc?.Sum(ct => ct.SoLuong) ?? 0);

        /// <summary>Tổng số quà đã phát/tặng = số đơn hàng có đổi quà bằng điểm.</summary>
        public int TongSoQuaDaTang() => DanhSachDonHang.Count(d => !string.IsNullOrEmpty(d.QuaTangDoi));

        /// <summary>Tên quà được đổi/tặng nhiều lần nhất (toàn bộ thời gian), rỗng nếu chưa có quà nào được tặng.</summary>
        public string QuaDuocTangNhieuNhat()
        {
            var top = DanhSachDonHang
                .Where(d => !string.IsNullOrEmpty(d.QuaTangDoi))
                .GroupBy(d => d.QuaTangDoi)
                .OrderByDescending(g => g.Count())
                .FirstOrDefault();

            return top != null ? $"{top.Key} ({top.Count()} lần)" : "-";
        }

        public int TongDiemDaCongToanBo() => DanhSachDonHang.Sum(d => d.DiemCong);

        public int TongDiemDaSuDungToanBo() => DanhSachDonHang.Sum(d => d.DiemSuDung);
    }
}