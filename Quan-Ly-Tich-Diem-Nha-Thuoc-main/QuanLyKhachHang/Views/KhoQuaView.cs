...
namespace QuanLyKhachHang.Views
{
    ...
    public class KhoQuaView : UserControl
    {
        ...
        public KhoQuaView(DataService data)
        {
            ...
            // Cho phép chọn nhiều quà cùng lúc (Ctrl/Shift+Click) ở 2 bảng "Chưa tặng" / "Đang tặng"
            // để chuyển trạng thái hàng loạt chỉ với 1 lần bấm nút.
            _listBoxChuaBan.SelectionMode = SelectionMode.Multiple;
            _listBoxDangBan.SelectionMode = SelectionMode.Multiple;
            ...
            _btnChuyenSangDangBan.Click += (s, e) => ChuyenNhieu(_listBoxChuaBan);
            _btnChuyenVeChuaBan.Click += (s, e) => ChuyenNhieu(_listBoxDangBan);
        ...
        private Border TaoGroupBoxTrangThai()
        {
            ...
            panelChuaBan.Children.Add(new TextBlock
            {
                Text = "💡 Giữ Ctrl (hoặc Shift) và bấm chọn để chọn nhiều quà cùng lúc",
                FontSize = 11,
            ...
            panelDangBan.Children.Add(new TextBlock
            {
                Text = "💡 Giữ Ctrl (hoặc Shift) và bấm chọn để chọn nhiều quà cùng lúc",
                FontSize = 11,
                Foreground = Brushes.DimGray
        ...
        /// <summary>
        /// Chuyển trạng thái cho TOÀN BỘ quà đang được chọn (có thể chọn nhiều bằng Ctrl/Shift+Click)
        /// ở bảng nguồn (Chưa tặng hoặc Đang tặng) chỉ với 1 lần bấm nút.
        /// Nếu không có mục nào đang chọn nhiều, vẫn hỗ trợ trường hợp cũ: dùng quà đang chọn đơn (_dangChon).
        /// </summary>
        private async void ChuyenNhieu(ListBox nguon)
        {
            var cuaSoCha = TopLevel.GetTopLevel(this) as Window;

            var dsChon = nguon.SelectedItems?.Cast<QuaTang>().ToList() ?? new List<QuaTang>();
            if (dsChon.Count == 0 && _dangChon != null)
                dsChon.Add(_dangChon);

            if (dsChon.Count == 0)
            {
                await ThongBaoWindow.ThongBao(cuaSoCha, "Thông báo", "Vui lòng chọn ít nhất 1 quà tặng cần chuyển trạng thái.");
                return;
            }
